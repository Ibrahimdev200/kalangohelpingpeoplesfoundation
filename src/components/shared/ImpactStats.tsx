      ...
      <div className="container-custom">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat, index) => (
            ...
            >
              <div className={`inline-flex p-4 rounded-full mb-4 ${dark ? "bg-background/10" : "bg-primary/10"}`}>
                <stat.icon className={`h-8 w-8 ${dark ? "text-background" : "text-primary"}`} />
              </div>
              <h3 className={`text-3xl md:text-4xl font-display font-bold mb-2 ${dark ? "text-background" : "text-foreground"}`}>
                {stat.value}
              </h3>
              <p className={`text-sm md:text-base ${dark ? "text-background/80" : "text-muted-foreground"}`}>
                {stat.label}
              </p>
            </div>
          ))}
